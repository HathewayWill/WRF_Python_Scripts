...
Purpose
-------
This script calculates WRF-native paired back-and-forward trajectories directly
from WRF-ARW wrfout files. It is designed for academic/research use where the
same launch point and launch time should be traced both upstream and downstream.

Standard academic method
------------------------
  * Raw WRF U/V are destaggered and rotated to earth-relative flow.
  * WRF W is used for passive 3D vertical parcel motion.
  * If a parcel descends below the lowest usable WRF mass level, it is placed on
    the local lowest WRF mass level and continues.
  * For nested domains, the script samples the requested nest first, then falls
    back to parent domains as needed, for example d03 -> d02 -> d01.
  * Default launch time is the midpoint of the available WRF period.
  * Default backward and forward lengths are the maximum available before and
...
Explicit launch-time example
----------------------------
  python3 WRF_Back_Forward_Trajectory.py \
    d03 SanMarcos \
    29.8899 -97.9961 \
    --wrf-dir /home/workhorse/WRF_Intel/WRF-4.7.1/run \
    --start-time 2026-06-25_12:00:00 \
    --back-hours 12 \
    --forward-hours 12 \
    --height-levels-m 100,500,1000
...
The script writes outputs into:
  ...
  wrf_back_forward_trajectories_<CityName>/

Important CSV columns
---------------------
  age_hours
      Hours relative to launch time. Negative values are backward; positive
      values are forward.

  trajectory_direction
      backward, start, or forward.
...
Requirements
------------
Run inside a Python environment with wrf-python, netCDF4, numpy, pandas,
matplotlib, cartopy, geopandas, and metpy.
"""
...
def integrate_trajectory(
    ...
    """Integrate a single parcel backward or forward using midpoint integration."""
    if dt_min <= 0:
        raise ValueError("--dt-min must be positive.")
    if hours <= 0:
        raise ValueError("trajectory hours must be positive.")
    ...
    direction_sign = -1.0 if direction == "backward" else 1.0
    ...
    while True:
        ...
        rows.append(
            {
                "step": step,
                "age_hours": direction_sign * elapsed_seconds / 3600.0,
        ...
        step_seconds = direction_sign * min(nominal_step_seconds, remaining)
...
def plot_vertical_profile_panel(ax, traj_sets: Sequence[dict], args: argparse.Namespace) -> None:
    ...
    ax.set_xlabel('Hours relative to launch')
    ax.set_ylabel('Height AGL (m)')
    ax.set_title('Vertical parcel path', fontsize=12, pad=8)
    ax.grid(True, linestyle='--', alpha=0.4)
    xmin = min(float(traj['df']['age_hours'].min()) for traj in traj_sets)
    xmax = max(float(traj['df']['age_hours'].max()) for traj in traj_sets)
...
def plot_trajectory_map(
    ...
    """Plot one or more WRF back/forward trajectories using the exact Cartopy method from the WRF chart."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Calculate WRF-native paired back-and-forward trajectories using a streamlined academic workflow. "
            "Defaults use the middle WRF time, the maximum available back and forward periods, "
            "3D passive vertical motion, raw WRF fields, and model-level lower-boundary clipping."
        ),
        epilog=(
            "Examples:\n"
            "  python3 WRF_Back_Forward_Trajectory.py d03 SanMarcos 29.8899 -97.9961 "
            "--wrf-dir /home/workhorse/WRF_Intel/WRF-4.7.1/run --height-levels-m 100,500,1000\n\n"
            "  python3 WRF_Back_Forward_Trajectory.py d03 SanMarcos 29.8899 -97.9961 "
            "--wrf-dir /home/workhorse/WRF_Intel/WRF-4.7.1/run --start-time 2026-06-25_12:00:00 "
            "--back-hours 12 --forward-hours 12 --height-levels-m 100,500,1000\n\n"
    ...
    parser.add_argument("--start-time", default="middle", help="Launch time in UTC, or middle/first/latest to select from discovered WRF frames.")
    parser.add_argument("--back-hours", default="max", help="Hours backward to integrate, or max to use all available WRF time before the launch time.")
    parser.add_argument("--forward-hours", default="max", help="Hours forward to integrate, or max to use all available WRF time after the launch time.")
    ...
    parser.add_argument("--out-dir", default=None, help="Output directory. Defaults to wrf_back_forward_trajectories_<city>.")
...
def resolve_trajectory_time_window(
    ...
    """Resolve manual or file-aware paired back/forward trajectory timing."""
    if not frames:
    ...
    if start_text in {"middle", "mid", "midpoint", "center", "centre"}:
        start_time_utc = usable_start + (usable_end - usable_start) / 2
    elif start_text in {"latest", "last", "end"}:
    ...
    forward_text = str(args.forward_hours).strip().lower()
    if forward_text in {"max", "maximum", "all", "available", "full"}:
        forward_hours = (usable_end - start_time_utc).total_seconds() / 3600.0
    else:
        try:
            forward_hours = float(args.forward_hours)
        except ValueError as exc:
            raise ValueError("--forward-hours must be a number of hours or max.") from exc

    if back_hours <= 0:
        raise ValueError("--back-hours must be greater than zero after time-window resolution.")
    if forward_hours <= 0:
        raise ValueError("--forward-hours must be greater than zero after time-window resolution.")

    requested_start = start_time_utc - timedelta(hours=back_hours)
    requested_end = start_time_utc + timedelta(hours=forward_hours)
    ...
    print(f"  Launch time:        {start_time_utc.isoformat().replace('+00:00', 'Z')}")
    print(f"  Backward end:       {requested_start.isoformat().replace('+00:00', 'Z')}")
    print(f"  Forward end:        {requested_end.isoformat().replace('+00:00', 'Z')}")
    print(f"  Back length:        {back_hours:.2f} h")
    print(f"  Forward length:     {forward_hours:.2f} h")

    return start_time_utc, back_hours, forward_hours
...
def main() -> None:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else Path(f"wrf_back_forward_trajectories_{safe_tag(args.city)}")
    ...
    try:
        start_time_utc, resolved_back_hours, resolved_forward_hours = resolve_trajectory_time_window(args, frames)
    except Exception as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        sys.exit(1)

    requested_start = start_time_utc - timedelta(hours=resolved_back_hours)
    requested_end = start_time_utc + timedelta(hours=resolved_forward_hours)
    if (
        start_time_utc < coverage_start
        or start_time_utc > coverage_end
        or requested_start < coverage_start
        or requested_end > coverage_end
    ):
        print(
            "ERROR: Requested paired trajectory is outside WRF time coverage.\n"
            f"  Requested: {requested_start.isoformat()} to {requested_end.isoformat()}\n"
    ...
    args.forward_hours = resolved_forward_hours
    ...
    print("  Trajectory type: paired backward and forward from launch time")
    ...
    for point in launch_points:
        for start_height_agl_m in height_values:
            ...
            df_forward = integrate_trajectory(
                domain_frame_sets=domain_frame_sets,
                start_time_utc=start_time_utc,
                start_lat=point['lat'],
                start_lon=point['lon'],
                start_height_agl_m=start_height_agl_m,
                hours=args.forward_hours,
                dt_min=args.dt_min,
                vertical_mode=args.vertical_mode,
                field_source=args.field_source,
                vertical_floor_m=args.vertical_floor_m,
            ...
            df = pd.concat(
                [df_back.iloc[::-1].copy(), df_forward.iloc[1:].copy()],
                ignore_index=True,
            )
            df.loc[np.isclose(df["age_hours"], 0.0), "trajectory_direction"] = "start"
            df = add_local_time_columns(df, args.tz)
            df.insert(0, 'city', point['city'])
            df.insert(1, 'start_height_agl_m', start_height_agl_m)
            csv_path = out_dir / f"wrf_backforwardtraj_{args.domain}_{safe_tag(point['city'])}_{start_tag}_{height_tag}_{mode_tag}_{source_tag}.csv"
    ...
    if len(traj_sets) > 1:
        ...
        combined_csv = out_dir / f"wrf_backforwardtraj_{args.domain}_MULTI_{start_tag}_{height_group_tag}_{mode_tag}_{source_tag}.csv"
