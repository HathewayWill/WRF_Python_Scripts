...
Purpose
-------
This script calculates WRF-native forward trajectories directly from WRF-ARW
...
The script:
  ...
  * automatically uses the first available WRF time as the trajectory start,
  * automatically uses the maximum available forward-trajectory length,
  * supports nested domains by falling back outward, for example d03 -> d02 -> d01,
  * calculates one or more forward trajectories from one or more launch locations,
...
The streamlined default method is:
  ...
  * Time window:
      Start at the first available WRF output time and integrate forward
...
Common optional arguments:
  ...
  --forward-hours max
      Default. Uses the maximum available forward period after the selected start.
...
The script writes outputs into:
  ...
  wrf_forward_trajectories_<CityName>/

Typical outputs include:

  wrf_fwdtraj_<domain>_<city>_<time>_<height>mAGL_passive_w_modelclip_raw.csv
  wrf_fwdtraj_<domain>_MULTI_<time>_levels_<heights>mAGL_passive_w_modelclip_raw.csv
  wrf_fwdtraj_<domain>_MULTI_<time>_levels_<heights>mAGL_passive_w_modelclip_raw.png
...
---------------------
  ...
  age_hours_forward
      Hours after the trajectory start time.
...
def integrate_back_trajectory(
    ...
    """Integrate a single parcel forward using midpoint integration."""
    if dt_min <= 0:
        raise ValueError("--dt-min must be positive.")
    if back_hours <= 0:
        raise ValueError("--forward-hours must be positive.")
    ...
    while True:
        ...
        rows.append(
            {
                "step": step,
                "age_hours_forward": elapsed_seconds / 3600.0,
...
def compute_trajectory_view_extent(
    ...
    forward-trajectory path plus enough padding to keep the track, nearby cities,
...
def add_direction_arrows(ax, df: pd.DataFrame, n_arrows: int = 3) -> None:
    """Draw arrows along the plotted line pointing forward in time."""
...
def plot_vertical_profile_panel(ax, traj_sets: Sequence[dict], args: argparse.Namespace) -> None:
    ...
    ax.set_xlabel('Hours forward')
    ax.set_ylabel('Height AGL (m)')
    ax.set_title('Vertical parcel path', fontsize=12, pad=8)
    ax.grid(True, linestyle='--', alpha=0.4)
    ax.set_xlim(0, max(float(traj['df']['age_hours_forward'].max()) for traj in traj_sets))
...
def plot_trajectory_map(
    ...
    """Plot one or more WRF forward trajectories using the exact Cartopy method from the WRF chart."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Calculate WRF-native forward trajectories using a streamlined academic workflow. "
    ...
    parser.add_argument("--start-time", default="first", help="Start time in UTC, or first/latest to select from discovered WRF frames.")
    parser.add_argument("--forward-hours", dest="back_hours", default="max", help="Hours forward to integrate, or max to use all available WRF time after the selected start time.")
    ...
    parser.add_argument("--out-dir", default=None, help="Output directory. Defaults to wrf_forward_trajectories_<city>.")
...
def resolve_trajectory_time_window(
    ...
    """Resolve manual or file-aware forward-trajectory timing from discovered WRF frames."""
    if not frames:
    ...
    if start_text in {"first", "earliest", "begin", "beginning", "start"}:
    ...
    return start_time_utc, forward_hours
...
def main() -> None:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else Path(f"wrf_forward_trajectories_{safe_tag(args.city)}")
    ...
    if (
        ...
        print(
            "ERROR: Requested forward trajectory is outside WRF time coverage.\n"
            f"  Requested: {start_time_utc.isoformat()} to {requested_end.isoformat()}\n"
    ...
    for point in launch_points:
        for start_height_agl_m in height_values:
            ...
            csv_path = out_dir / f"wrf_fwdtraj_{args.domain}_{safe_tag(point['city'])}_{start_tag}_{height_tag}_{mode_tag}_{source_tag}.csv"
    ...
    if len(traj_sets) > 1:
        ...
        combined_csv = out_dir / f"wrf_fwdtraj_{args.domain}_MULTI_{start_tag}_{height_group_tag}_{mode_tag}_{source_tag}.csv"
